...
(defun vimpulse-concatenate-lists (&rest sequences)
...
(when (fboundp 'font-lock-add-keywords)
  (font-lock-add-keywords
   'emacs-lisp-mode
   '(("(\\(vimpulse-\\(?:with\\|save\\)-[-[:word:]]+\\)\\>" 1 font-lock-keyword-face))))
...
(defun vimpulse-transient-mark-p ()
  "Whether Transient Mark mode is enabled."
  (if (featurep 'xemacs)
      zmacs-regions
    transient-mark-mode))
...
Enable with positive ARG, disable with negative ARG."
  (unless (numberp arg)
    (setq arg (if (vimpulse-transient-mark-p) -1 1)))
  (cond
   ((< arg 1)
    (when (vimpulse-region-active-p)
...
(defun vimpulse-region-active-p ()
  "Whether the region is active."
  (if (featurep 'xemacs)
      (vimpulse-region-active-p)
    (and transient-mark-mode mark-active)))

(defun vimpulse-active-region (&optional arg)
  "Toggle active region.
Enable with positive ARG, disable with negative ARG."
  (unless (numberp arg)
    (setq arg (if (vimpulse-region-active-p) -1 1)))
  (cond
   ((and (< arg 1))
    (when (vimpulse-region-active-p)
      (if (featurep 'xemacs)
   ...
   (t
    (unless (vimpulse-transient-mark-p)
      (vimpulse-transient-mark 1))
    (when deactivate-mark
      (setq deactivate-mark nil))
    (unless (mark t)
      (vimpulse-set-mark (point)))
    (unless (vimpulse-region-active-p)
      (if (featurep 'xemacs)
          (zmacs-activate-region)
        (set-mark (mark t)))))))

(defun vimpulse-set-mark (pos &optional activate)
  "Set buffer's mark to POS.
Do not activate the mark unless ACTIVATE is non-nil."
...
(eval-and-compile
  ...
  (defalias 'vimpulse-overlay-live-p
    (if (featurep 'xemacs) 'extent-live-p 'overlay-buffer))
...
(defun vimpulse-make-overlay
  ...
  (cond
   ;; the XEmacs equivalent is an "extent"
   ((featurep 'xemacs)
...
(defun vimpulse-set-overlay
  ...
  (when properties
    (apply 'vimpulse-overlay-put overlay properties))
  overlay)

(defun vimpulse-overlay-put (overlay prop value &rest properties)
  "Set property PROP of overlay OVERLAY to VALUE.
Additional property-value pairs may be given via PROPERTIES."
  (if (featurep 'xemacs)
      (set-extent-property overlay prop value)
    (overlay-put overlay prop value))
  (while properties
    (vimpulse-overlay-put overlay (pop properties) (pop properties))))
